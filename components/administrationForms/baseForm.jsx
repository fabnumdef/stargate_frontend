...
const PaginationButton = ({ users, fetchMore, selectList, setSelectList }) => {
    ...
    return (
        users.list.length < users.meta.total && (
...
const BaseForm = ({ submitForm, defaultValues, setDefaultValues, usersList, fetchMore, type }) => {
    ...
    const findCampusAdmin = () =>
        selectList.listUsers.list.find((user) => user.id === defaultValues.admin.id) || '';
                            ...
                            <Controller
                                as={
                                    <Select
                                        ...
                                        {selectList &&
                                            selectList.listUsers.list.map((user) => (
                        ...
                        <FormControl className={classes.assistantSelect}>
                            <TableContainer className={classes.tableContainer}>
                                <Table size="small" data-testid="placeTable">
                                    <TableBody>
                                        {assistantsList[FORMS_LIST.ADMIN_ASSISTANTS].map(
                                            (user, index) =>
                                                !user.toDelete && (
                                                    <TableRow
                                                        key={user.id}
                            ...
                            <Select
                                ...
                                {selectList &&
                                    selectList.listUsers.list.map(
                                        (user) =>
                                            (!watch('campusAdmin') ||
                                                watch('campusAdmin').id !== user.id) && (
                                                <MenuItem key={user.id} value={user}>
                                                    <Checkbox
                                                        checked={
                                                            assistantsList[
