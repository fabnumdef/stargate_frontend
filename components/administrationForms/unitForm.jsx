...
const UnitForm = ({ defaultValues, type, submitForm, campusId }) => {
    ...
    const getPlacesList = async () => {
        try {
            ...
            return addAlert({
                message:
                    'Erreur lors du chargement de la liste de lieux, merci de rafraichir la page',
                severity: 'warning'
            });
        }
    };
    ...
    const isAssistant = (userId, typeAssistant) =>
        assistantsList[typeAssistant].find((user) => user.id === userId && !user.toDelete);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className={classes.createUnitForm}>
            ...
            <Grid container item sm={12} xs={12}>
                        ...
                        <Grid className={classes.textFieldBlock}>
                            ...
                            <Grid container alignItems="flex-end" justify="space-between">
                                <Controller
                                    ...
                                    name="trigram"
                                    defaultValue={
                                        defaultValues.trigram ? defaultValues.trigram : ''
                                    }
                                />
                                <Grid>
                                    <Typography style={{ fontStyle: 'italic' }}>
                                        Trigramme de l&apos;unité, 2 à 3 lettres uniques et non
                                        editable par la suite
                                    </Typography>
                                </Grid>
                            </Grid>
                        </Grid>
                ...
                <Grid item sm={6} xs={6}>
                    <Grid className={classes.sectionContainer}>
                        <Grid>
                            <Grid item md={5} sm={12} className={classes.titleUserSelect}>
                            ...
                            <Grid
                                ...
                                md={7}
                                sm={12}
                                className={`${matches ? classes.selectMargin : ''} ${
                                    classes.userSelect
                                }`}>
                                <FormControl
                                    variant="outlined"
                            ...
                            <Grid className={classes.userSelect}>
                                <FormControl className={classes.assistantSelect}>
                                    {assistantsList[FORMS_LIST.CORRES_ASSISTANTS].map(
                                        (user) =>
                                            !user.toDelete && (
                                                <DeletableList
                                                    key={user.id}
                                    ...
                                    <Select
                                        ...
                                        onChange={(evt) =>
                                            addAssistant(evt, FORMS_LIST.CORRES_ASSISTANTS)
                                        }>
                                        {usersList &&
                                            usersList.listUsers.list.map(
                                                (user) =>
                    ...
                    <Grid className={classes.sectionContainer}>
                        <Grid>
                            ...
                            <Grid className={classes.userSelect}>
                                <FormControl className={classes.assistantSelect}>
                                    {assistantsList[FORMS_LIST.OFFICER_ASSISTANTS].map(
                                        (user) =>
                                            !user.toDelete && (
                                                <DeletableList
                                                    key={user.id}
                                    ...
                                    <Select
                                        ...
                                        renderValue={() => <em>optionnel</em>}
                                        onChange={(evt) =>
                                            addAssistant(evt, FORMS_LIST.OFFICER_ASSISTANTS)
                                        }>
                                        {usersList &&
                                            usersList.listUsers.list.map(
                                                (user) =>
