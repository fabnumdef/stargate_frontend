...
function getMenus(router, setSubMenuAdmin, subMenuAdmin) {
    return [
        { label: 'Mes Traitements', permission: '/', action: () => router.push('/') },
        {
            ...
            action: () => router.push('/mes-demandes')
        },
        {
            label: 'Administration',
            permission: '/administration',
            action: () => setSubMenuAdmin(!subMenuAdmin)
        }
    ];
}
    ...
    return [
        {
            ...
            action: () => router.push('/administration/utilisateurs')
        },
        {
            label: 'Unités',
            permission: '/administration/unites',
            action: () => router.push('/administration/unites')
        },
        {
            label: 'Base',
            permission: '/administration/base',
            action: () => router.push(`/administration/base/${campusId}`)
        }
    ];
}
...
export default function MenuItems() {
    ...
    const checkActiveButton = (permission) =>
        router.pathname === permission ||
        (router.pathname.includes(permission) && permission !== '/');
                        ...
                        urlAuthorization(permission, activeRole.role) && (
                            <Grid key={label}>
                                <ButtonMenu
                                    ...
                                    variant="contained"
                                    aria-haspopup="true"
                                    color={checkActiveButton(permission) ? 'secondary' : 'primary'}
                                    onClick={label === 'Administration' ? adminClick : action}>
                                    {label}
                                </ButtonMenu>
                                {label === 'Administration' && (
                                    <Menu
                                        ...
                                        onClose={adminClose}>
                                        {campus &&
                                            getAdminMenu(router, campus.campusId).map(
                                                (subMenu) =>
                                                    urlAuthorization(
                                                        subMenu.permission,
                                                        activeRole.role
