...
const checkWithUnit = (units, activeRole) => {
    ...
    if (unit) {
        ...
        if (
            unit.steps[userIndex].state.value ||
            previousSteps.find(
                (s) =>
                    s.behavior === WORKFLOW_BEHAVIOR.VALIDATION.value &&
...
const checkWithoutUnit = (units, activeRole) => {
    const sortRejected = units.filter((u) =>
        u.steps.every(
            (step) =>
    ...
    const isActiveStep = units.find((u) =>
        u.steps.find(
            (step, index) =>
                step.role === activeRole.role &&
                (index === 0 ||
