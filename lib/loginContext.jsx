...
export function LoginContextProvider({ children }) {
    ...
    const [isLoggedUser, setIsLoggedUser] = useState(!tokenDuration().expiredToken);
    ...
    const [getUserData, { data: meData, loading: meLoading, error: meError }] = useLazyQuery(
        GET_ME
    );
    ...
    useEffect(() => {
        ...
        if (
            ...
            (isCacheInit && !urlAuthorization(router.pathname, activeRole.role))
        ) {
    ...
    return (
        <LoginContext.Provider
            ...
            (!isLoggedUser && router.pathname !== '/login') ||
            (activeRole && !urlAuthorization(router.pathname, activeRole.role))
                ? ''
                : children}
        </LoginContext.Provider>
    );
